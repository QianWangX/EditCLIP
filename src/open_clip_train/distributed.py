...
def get_model_attribute(model, attr_name):
    ...
    if hasattr(model, "module"):
        return getattr(model.module, attr_name)  # Access the underlying module
    else:
        return getattr(model, attr_name)  # Single-GPU case
    
# Helper function to safely set model attributes
def set_model_attribute(model, attr_name, value):
    if hasattr(model, "module"):  # Check if model is wrapped in DDP
